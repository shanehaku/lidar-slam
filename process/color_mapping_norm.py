...
def main():
    ...
    RAW_LIDAR_DIR = f"{target}/data/raw_pcd"
    LIDAR_POSE_PATH = f"{target}/result/lidar_poses.csv"

    # Image & Pose for Colorization
    IMG_DIR = f"{target}/data/image"
    IMG_POSE_PATH = f"{target}/result/camera_poses.csv"

    # Output
    OUTPUT_PCD = f"{target}/result/{target}.pcd"
